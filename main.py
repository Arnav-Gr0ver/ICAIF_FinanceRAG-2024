...
for task_name, task_instance in tasks.items():
    ...
    result_df = pd.DataFrame(reranking_result, columns=['corpus_id'])
    result_df['query_id'] = task_name  # Add a column for the task name

    # Reorder columns to have 'task' first and then 'result'
    result_df = result_df[['query_id', 'corpus_id']]
